...
def do_train_stage2(cfg,
    ...
    xent = SupConLoss(device)  # hiện tại không dùng, giữ lại cho đúng với code gốc

    # ==== Chuẩn bị ====
    all_start_time = time.monotonic()

    # Tính text_features một lần như CLIP-ReID gốc
    ...
    with torch.no_grad():
        ...
        # dùng float32 để tránh lỗi dtype với AMP
        text_features = torch.cat(text_features, 0).cuda().float()

    # Cờ dùng cross-attention ở Stage2
    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, (img, vid, target_cam, target_view) in enumerate(train_loader_stage2):
            ...
            img = img.to(device)
            target = vid.to(device)  # train_collate_fn trả về tensor
                ...
                if dist.get_rank() == 0:
                    ...
                    for n_iter, (img, vid, camid, camids, target_view, _) in enumerate(val_loader):
                        with torch.no_grad():
                            ...
                            # Eval: KHÔNG dùng cross-attention 
                            feat = model(
                                img,
                                cam_label=camids,
                                view_label=target_view,
    ...
    # ==== FINAL VALIDATION (tùy, có thể giữ cho báo cáo) ====
